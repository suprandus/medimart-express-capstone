        ...
        <div class="card" style="height: 70vh;">
          ...
          <div class="card-body">
            <ul class="list-unstyled list-unstyled-border">
              ...
              @php
              $unseenMessages = \App\Models\Chat::where(['sender_id' => $chatUser->receiverProfile->id, 'receiver_id' =>
              auth()->user()->id, 'seen' => 0])->exists();
              @endphp
              <li class="media chat-user-profile" data-id="{{ $chatUser->receiverProfile->id }}">
                <img alt="image" class="mr-3 rounded-circle {{ $unseenMessages ? 'msg-notification' : '' }}" width="50"
                  src="{{ asset($chatUser->receiverProfile->image) }}">
                <div class="media-body">
                  <div class="mt-0 mb-1 font-weight-bold chat-user-name">{{ $chatUser->receiverProfile->name }}</div>
                </div>
              </li>
              @endforeach
            </ul>
